    ...
    <div className="p-4 w-full">
      <div className="max-w-full mx-auto">
        <div className="bg-white rounded-xl shadow-md border border-gray-200 p-4 dark:bg-gray-800 dark:border-gray-700">
          <h1 className="text-xl font-bold text-gray-900 mb-4 dark:text-white">
            Daftar File
          </h1>
              ...
              <table className="w-full text-sm table-fixed">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-600">
                    <th className="text-left py-3 px-4 text-gray-700 dark:text-gray-300 w-64">Nama File (klik untuk lihat file)</th>
                    ...
                    <th className="text-left py-3 px-4 text-gray-700 dark:text-gray-300 w-48">Uploader</th>
                ...
                <tbody>
                  {files.map((file) => (
                    <tr key={file.id} className="border-b hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-700/50">
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-white truncate">{file.title}</td>
                      <td className="py-3 px-4">
                      ...
                      <td className="py-3 px-4">
                        {file.owner ? (
                          <div>
                            <div className="font-medium text-gray-900 dark:text-white truncate">{file.owner.full_name}</div>
                            <div className="text-gray-500 text-xs dark:text-gray-400 truncate">{file.owner.email}</div>
                          </div>
                        ) : (
                      ...
                      <td className="py-3 px-4">
                        {file.target_user ? (
                          <div>
                            <div className="font-medium text-gray-900 dark:text-white truncate">{file.target_user.full_name}</div>
                            <div className="text-gray-500 text-xs dark:text-gray-400 truncate">{file.target_user.email}</div>
                          </div>
                        ) : (
